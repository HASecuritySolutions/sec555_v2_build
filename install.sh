...
alias curator="docker run -it --rm --net=bridge --network=labs_esnet -v /etc/localtime:/etc/localtime:ro --name curator --hostname curator -v /labs/curator:/labs/curator:ro -e TZ=America/Chicago --link elasticsearch hasecuritysolutions/elastic_cron:v1.0 /usr/local/bin/curator"
